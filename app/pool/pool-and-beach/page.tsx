...
const page = POOL_PAGES.find((p) => p.slug === "pool-and-beach");
...
export default function PoolAndBeachPage() {
  if (!page) {
    notFound();
  }
  ...
  return (
    <main className="bg-gradient-to-b from-sky-50 via-white to-white text-slate-900">
      <section className="relative overflow-hidden border-b border-sky-100 bg-gradient-to-br from-sky-50 via-white to-emerald-50">
        <div className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-amber-200/70 blur-3xl" />
        ...
        <div className="mx-auto max-w-5xl px-6 py-16">
          ...
          <div className="mt-6 flex items-start gap-4">
            <div className="mt-1 grid h-12 w-12 place-items-center rounded-2xl bg-amber-700 text-white shadow-sm">
            ...
            <section>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-3">
                {page.highlights.map((item) => (
                  <div
                    key={item}
                    className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-900 shadow-sm"
                  >
                    {item}
                  </div>
                ))}
