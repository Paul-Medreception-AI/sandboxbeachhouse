...
const page = POOL_PAGES.find((p) => p.slug === "heated-pool");
...
export default function HeatedPoolPage() {
  if (!page) {
    notFound();
  }
      ...
      <section className="relative overflow-hidden border-b border-sky-100 bg-gradient-to-br from-sky-50 via-white to-emerald-50">
        ...
        <div className="mx-auto max-w-5xl px-6 py-16">
          ...
          <div className="mt-6 flex items-start gap-4">
            <div className="mt-1 grid h-12 w-12 place-items-center rounded-2xl bg-sky-700 text-white shadow-sm">
            ...
            <section>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-3">
                {page.highlights.map((item) => (
                  <div
                    key={item}
                    className="rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm font-medium text-sky-900 shadow-sm"
                  >
                    {item}
                  </div>
                ))}
