...
import { getSeasonalPage, seasonalPages } from "./data";
...
function monthToSlug(monthName: string, today = new Date()) {
...
export function getSeasonalMetadata(slug: string): Metadata {
...
export function getSeasonalSlugs() {
  return seasonalPages.map((page) => page.slug);
}

export default function SeasonalTopic({ slug }: { slug: string }) {
  const page = getSeasonalPage(slug);
        ...
        <div className="mx-auto max-w-5xl px-6 py-16">
          <nav className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500" aria-label="Breadcrumb">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Link href="/seasonal/" className="hover:text-slate-700">
                Seasonal
              </Link>
              <span className="text-slate-300">/</span>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-5xl px-6">
          <div className="grid gap-10">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-2xl font-semibold text-slate-900">Timing and open dates</h2>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-2xl font-semibold text-slate-900">Sample stay rhythm</h2>
              <ul className="mt-4 space-y-3 text-lg text-slate-700">
                {page.itinerary.map((step) => (
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-2xl font-semibold text-slate-900">Local tips</h2>
              <div className="mt-4 space-y-3 text-lg text-slate-700">
            ...
            {page.monthHints.length > 0 && (
              <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 className="text-2xl font-semibold text-slate-900">Plan by month</h2>
                <div className="mt-4 flex flex-wrap gap-3">
                  {page.monthHints.map((month) => {
                    const slugValue = monthToSlug(month);
                    if (!slugValue) return null;
                    return (
                      <Link
                        key={month}
                        href={`/open-dates/month/${slugValue}`}
                        className="rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-900 shadow-sm transition hover:border-emerald-200 hover:bg-emerald-50"
                      >
                        {month} open dates
                      </Link>
                    );
            ...
            <section className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-900 to-emerald-900 p-8 text-white shadow-sm">
